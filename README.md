...
You can also force the Node Placeholder service to always respect calendar entries and override any reductions based on existing resources.  This is useful when you have a deployment that typically has enough resources available before a surge login event, and the node is underutilized leading to the service automatically reducing the number of replicas.  This is disabled by default, and can be changed by setting the value of `calendarOverrideEnabled` to `True` in `values.yaml`.
...
calendarOverrideEnabled: True # default is False
